...
$$
and $\tilde{V}_i(s)=p_{0j}(0,s)\indic{i=j}$ providing the initial condition for the Kolmogorov forward differential equations. The differential equation for the retrospective reserve for $s<t$ is given by 
\begin{gather*}
...
\end{align*}
for $g$ and $h$ functions that are affine in all entries of $W$, and determined by the dynamics of $X$ and $Y$. A function $f$ is affine in all entries of $W$ if and only if
$$
...
$$
As $h^{Z(s-)k}(s,W(s-)$ is predictable, we may replace the integrator $dN^k(s)$ with its predictable compensator. Using the tower property once more,
\begin{align*}
\tilde{W}^i(t)=& p_{0i}(0,t)W(0)+
\int_0^t \E_0 \left[ \E_0 \left[ \indic{Z(t)=i} g^{Z(s)}(s,W(s))|Z(s) \right]\right] ds
\\
&+
...
\\
&+
\sum_{j \in \mathcal{J}} p_{0j}(0,s)   \E_0 \left[\int_0^t \sum_{k \neq j} \indic{Z(t)=i} h^{jk}(s,W(s-))  dN^k(s) |Z(s-)=j \right]
\displaybreak \\
=&
p_{0i}(0,t)W(0)+
\int_0^t \sum_{j \in \mathcal{J}} p_{0j}(0,s) \E_0 \left[ \indic{Z(t)=i} g^{Z(s)}(s,W(s))|Z(s)=j\right] ds
...
\\
&+
\sum_{j \in \mathcal{J}} p_{0j}(0,s)  \int_0^t \sum_{k \neq j }   \E_0 \left[ h^{jk}(s,W(s-))  |Z(s-)=j,Z(t)=i \right] \mu_{jk}(s)p_{ki}(s,t) ds.
\end{align*}
