...
%\\
%=&
%\E^* \left[ \int_0^t e^{\int_s^t r*}  \left( \sum_{i:i \in \mathcal{J}} \indic{Z(s)=i} %b^i(s)ds+ \sum_{k \neq Z(s-)} b^{ik}(t) dN^k(s) \right) \big| \mathcal{E}_t \right]
\end{align*} 
...
%\\ =& \frac{-1}{p_{0i}(0,t)}\int_0^t e^{\int_s^t r*} \sum_{j:j \in \mathcal{J}} p_{0j}(0,s)p_{ji}(s,t) \left(  b^{j}(s) + \sum_{k \neq j}  \mu_{jk}(s)b^{jk}(s)\frac{p_{ki}(s,t)}{p_{ji}(s,t)} \right) ds
 \nonumber \\
=&
...
\\
=&
\int_{(0,t]}  \sum_{i:i \in \mathcal{J}} \frac{p_{ig}(\tau,t)}{p_{0g}(0,t)} \sum_{k \neq i} \phi(\tau,t,i,k)\tilde{X}^i(\tau) \mu_{ik}(\tau)p_{kj}(\tau,r) d\nu(\tau,t)
\end{align*}
