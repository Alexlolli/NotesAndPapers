...
\nonumber \\
&+
\tilde{W}^i(t)g_1(t,i)+p_{Z(0)i}(0,t)g_2(s,i)
\nonumber \\
&+
...
\end{align}
\end{thm}
The differential equations given by \eqref{eq:AAF}-\eqref{eq:AAG} bear close resemblance to the differential equation for the retrospective reserve derived by \citet{Norberg}. In contrast to \citet{Norberg} we allow for dynamics of the process to depend on the process itself. To get a feel for the individual terms of \eqref{eq:AAF}, we write out the differential equation for $\tilde{W}^i$ when $W(t)=(X(t),Y(t))^T$ and the dynamics of $X$ and $Y$ are given by \eqref{eq:AAB}-\eqref{eq:AAC}.
