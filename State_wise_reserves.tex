...
In its simplicity, the contribution of this article is to embrace the fact that we, as humans, make decisions based on our past. Even though I twist the wise words of Søren Kierkegaard, they 
...
\end{pmatrix},
\\
\frac{d}{dt}\hat{W}(t)=&p_{aa}(0,t) g \left( s,0,\frac{\hat{W}(t)}{p_{aa}(0,t)}\right)
-
\frac{\hat{W}(t)}{p_{aa}(0,t)}\sum_{k=1}^n \mu_{0k}(t).
\end{align*}
...
\subsection*{Two-active-states model}
We expand to a two-state hierarchical model.Illustrér at linaritet er nødvendig i transient model.

\begin{align*}
W_0(t)=&\int_0^t g(s,0,W_1(s)) ds
\\
...
\end{align*}
The function $q$ represents the intensity of transition at time $s$, given transition between $0$ and $t$. Let $T_1$ be the time of the first jump.
\begin{align*}
