...
From a mathematical point of view, they pose a problem as they depend on the entire history of the portfolio of policies in a possibly non-linear fashion, making it difficult to calculate prospective reserves. If we want to take a glance into the crystal ball of liabilities, taking Future Management Actions (FMA's) into account, we need to embrace it's retrospective nature. In this paper, we do not incorporate FMA's to their full extent, but rather lay the groundwork on which models including FMA's can be built.
\\[12pt]
...
The savings $X$, and surplus $Y$, are the main quantities of interest. In practice, the dynamics of these accounts are very specific, and not a subject of debate. However, instead of working with the explicit dynamics of $X$ and $Y$, we choose to use the more general affine dynamics given by
\begin{align*}
