...
Syvtilstandsmodel. Forklar hvad der sker ved overgang til fripolice. Bemærk $V^{3*}(t)=V^{0*+}(t)$.\\

The free policy option is the option for the insured to cease all future premiums in exchange all future benefits being scaled accordingly. In the case where benefits are not scaled according to the savings account, there is a natural way to calculate the scaling factor, also called the free policy factor, as the factor that solves
\begin{align*}
...
\end{gather*}
It is worth noting that this choice of free policy factor leads to $R^{03}(t,x)=b^{03}(t,x)=0$. As we assume that the benefits are defined $dB_2^+=dB_2$,
$$
f(t)=\frac{X(t)}{X(t)-V_1^{0*-}(t)}.
$$
Furthermore, the only duration dependent part of the benefit pertains to the payments that are not scaled by the savings account
\begin{align*}
...
\end{align*}
Note the very important special cases where $g_3=0$, when $dB_1^{Z(t)+}(t)=dB_2^{Z(t)+}(t)$ or $dB_1^{Z(t)+}(t)=0$. In these cases, the dynamics of $X$ are independent of the free-policy duration. This is true because the otherwise duration dependent terms of the dynamics of $X$, $dB$ and $\chi$ can be written as
\begin{align*}
dB^i(t,x,u)=&
f(t-u) \underbrace{\left( dB^{i+}_2(t) - dB_2^{i+}(t) \right)}_{=0}  +\frac{x}{V_2^{i*+}(t)}dB_2^{i+}(t)
\\
\chi^{jk}(t,x,u)=&V_1^{k*+}(t)f(t-u)+\frac{x-V_1^{j*+}(t)f(t-u)}{V_2^{j*+}(t)f(t-u)}V_2^{k*+}(t)f(t-u)
\\
=&
V_2^{k*+}(t)f(t-u)+\frac{x-V_2^{j*+}(t)f(t-u)}{V_2^{j*+}(t)f(t-u)}V_2^{k*+}(t)f(t-u)
\\
=&
